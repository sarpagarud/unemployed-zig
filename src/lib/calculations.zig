...
pub fn velocity_of_money(P: f32, Q: f32, M: f32) f32 {
  const V = (P*Q)/M;
  return V;
}
//
...
//
pub fn velocity_of_money_all(P: i32, T: i32, M: i32) i32 {
  const Vt = (P*T)/M;
  return Vt;
}
...
//
pub fn inflation_cpi(previous: f64, current: f64) f64 {
...
//
pub fn quantity_theory_of_money(M: i32, V: i32, P: i32, Q: i32) i32 {
  return (M*V)==(P*Q);
}
...
//
pub fn producer_price_index(previous: f64, current: f64) f64 {
  return ((current - previous)/previous) * 100;
}
